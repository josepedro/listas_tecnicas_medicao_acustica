%% Cálculo do coeficiente de absorção sonora de uma amostra de material
%% acústico
...
H12_I = load('medicaoladerocha1_pos1.txt');

H12_II = load('medicaoladerocha1_pos2.txt');
...
title('\bf{Curvas de coficiente de absorção sonroa e de reflexão da amostra de material acústico ensaiada}', 'fontsize', 22);
ylabel('\bf{Amplitude dos coeficientes }', 'fontsize', 18);
xlabel('\bf{Frequência [Hz]}', 'fontsize', 18);
legend('Coeficiente de absorção sonora \bf{\alpha}','Coeficiente de reflexão \bf{r}','fontsize', 14);
xlim([fmin 7000]);
ylim([0 1]);
grid on
